<template>
  <div class="des_lj">
    ...
    <Destinationdev />
    <!-- 丽江风情 -->
    <Destinationplace :placeInfo="lj_Info"/>
  </div>
</template>
...
import { getDes_lj } from "@/api/getData.js";
...
export default {
  ...
  methods: {
    async ljInfo() {
      ...
      try {
        // 等待异步方法执行完成
        const result = await getDes_lj();
        console.log("目的地 lj 数据", result.data.data);
        this.lj_Info = result.data.data;
      } catch (err) {
        console.log("err", err);
      }
    },
